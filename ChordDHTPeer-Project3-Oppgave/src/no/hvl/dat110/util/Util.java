...
public class Util {
	...
	public static boolean computeLogic(BigInteger id, BigInteger lower, BigInteger upper) {
		...
		boolean cond = false;
		BigInteger bu = upper; //lagra upper verdien i bu
		BigInteger addS = Hash.addressSize();
		if (lower.compareTo(upper) == 1) { // altså lower > id og upper
			bu = upper.add(addS); //bu = upper+addressSize (id = upper men lower > id)
			
			//id <= upper
			if ((id.compareTo(upper) == -1 || id.compareTo(upper) == 0)) {
				id = id.add(addS);
			}
		}
		
		// lower4 < id4 <= upper4 (true)
		upper = bu; //fordi --> id = upper, men lower > id
		cond = (id.compareTo(lower) == 1 || id.compareTo(lower) == 0) && (id.compareTo(upper) == -1

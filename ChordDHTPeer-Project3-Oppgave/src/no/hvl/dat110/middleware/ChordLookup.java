...
public class ChordLookup {
	...
	public NodeInterface findSuccessor(BigInteger key) throws RemoteException {
		...
		NodeInterface succNode = this.node.getSuccessor();
		// get the stub for this successor (Util.getProcessStub())
		NodeInterface niStub = Util.getProcessStub(succNode.getNodeName(), succNode.getPort());
		...
		if (niStub != null) {
			...
			boolean condition = Util.computeLogic(key, nodeid.add(BigInteger.ONE), niStub.getNodeID());
	...
	private NodeInterface findHighestPredecessor(BigInteger key) throws RemoteException {
		...
		List<NodeInterface> entries = this.node.getFingerTable();
		// starting from the last entry, iterate over the finger table
		for(int i = 0; i < entries.size(); i++) {
		// for each finger, obtain a stub from the registry
			int last = (entries.size()-1) - i; //siste noden
			NodeInterface fingr = entries.get(last);
			NodeInterface stub = Util.getProcessStub(fingr.getNodeName(), fingr.getPort());
		// check that finger is a member of the set {nodeID+1,...,ID-1} i.e. (nodeID+1
		// <= finger <= key-1) using the ComputeLogic
			boolean condition = Util.computeLogic(fingr.getNodeID(), this.node.getNodeID().add(BigInteger.ONE), key.subtract(BigInteger.ONE));
